...
def figure_fn(inp, gt, pred, size=[256, 256]):
    if inp.shape[1] == 3:
        ...
        for i in range(4):
            #! For displaying purpoe prolly, the reverse of the pre-processing is happening over here
            input_img = inp[i] * 0.5 + 0.5
            mean = torch.mean(input_img, dim=[1, 2])
            std = torch.std(input_img, dim=[1, 2])
            print(torch.max(equalize(input_img)), torch.min(equalize(input_img)))
            plt.subplot(4, 4, i * 4 + 1)
            plt.axis("off")
            # plt.imshow((equalize(input_img) * 255).permute(1, 2, 0).long().cpu().detach().numpy())
...
def train(train_loader, model, optimizer, loss_fn, epoch):
    ...
    for batch in tqdm(train_loader, leave=False, desc="train"):
        ...
        print(inp.shape, "Input Shape to the model")  #! Not needed Shoudl delete later
        pred = model(inp)
