...
from .unet_wacv import UNet2, UNetBig
...
@register('cfnet')
class cfnet(nn.Module):
    def __init__(self, maxdisp, use_concat_volume=True, unet='unet-basic', in_channels=3, out_channels=3):
        super(cfnet, self).__init__()
        self.maxdisp = maxdisp
        self.use_concat_volume = use_concat_volume
        self.v_scale_s1 = 1
        ...
        if unet=='unet-basic':
            ...
            self.unet = UNet2(in_channels=in_channels, out_channels=out_channels)
        elif unet=='unet-wacv-big':
            self.unet = UNetBig(in_channels=in_channels, out_channels=out_channels)
    ...
    def forward(self, x):
        ...
        left, right = x[:,:int(in_channels/2),:,:], x[:,int(in_channels/2):,:,:]
        ...
        if self.training:
            ...
            disps = [pred0_4, pred1_4, pred2_s4, pred0_s3, predmid_s3, pred1_s3_up, pred0_s2, predmid_s2, pred1_s2]
            recons = [self.image_warp(right, x) for x in disps]
            enhance_out = self.unet(torch.concat([left, recons[-1]], dim=1))
            ...
            recon = self.image_warp(right, pred1_s2)
            enhance_out = self.unet(torch.concat([left, recon], dim=1))
            return {'recons': [recon], 'pred': enhance_out}


@register('cfnet-pretrained')
class CFNetPretrained(nn.Module):
    def __init__(self, pretrained_path, maxdisp=256, use_concat_volume=True, unet='unet-basic', in_channels=3, out_channels=3):
        super().__init__()

        self.cfnet = CFNet(maxdisp=maxdisp, use_concat_volume=use_concat_volume)
        self.cfnet.load_state_dict(torch.load(pretrained_path))
        self.cfnet.eval()
    ...
    def forward(self, x):
        ...
        disps = self.cfnet(left, right)
        recon_left = self.image_warp(right, disps[0])
        enhance_out = self.unet(torch.concat([left, recon_left], dim=1))
        return enhance_out
